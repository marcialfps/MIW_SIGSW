...
var puntoControl = new google.maps.LatLng(43.35481, -5.851805);
const TILE_SIZE = 256;
...
function featureInfoWMS(pixelX, pixelY, tileX, tileY) {
  ...
  let myURL =
    stations_WMS_url + "SERVICE=WMS&VERSION=1.1.1&REQUEST=GetFeatureInfo";
...
function parserDataFeatureInfo(data) {
  ...
  if (xmlDoc.getElementsByTagName("ServiceException")[0] != undefined) {
    ...
    xmlDoc.getElementsByTagName("FIELDS")[0] != undefined &&
    xmlDoc.getElementsByTagName("FIELDS")[0].attributes != undefined
  ) {
    document.getElementById("data").innerHTML =
      "<ul>" +
      "<li><strong>" +
      "Código Nacional: " +
      "</strong>" +
      xmlDoc.getElementsByTagName("FIELDS")[0].attributes[2].nodeValue +
      "</li>" +
      "<li><strong>" +
      "Año: " +
      "</strong>" +
      xmlDoc.getElementsByTagName("FIELDS")[0].attributes[3].nodeValue +
      "</li>" +
      "<li><strong>" +
      "Código Europeo: " +
      "</strong>" +
      xmlDoc.getElementsByTagName("FIELDS")[0].attributes[4].nodeValue +
      "</li>" +
      "<li><strong>" +
      "Nombre de la estación: " +
      "</strong>" +
      xmlDoc.getElementsByTagName("FIELDS")[0].attributes[5].nodeValue +
      "</li>" +
      "<li><strong>" +
      "Valor legislado: " +
      "</strong>" +
      xmlDoc.getElementsByTagName("FIELDS")[0].attributes[6].nodeValue +
      "</li>" +
      "<li><strong>" +
      "Datos: " +
      "</strong>" +
      xmlDoc.getElementsByTagName("FIELDS")[0].attributes[10].nodeValue +
      "</li>" +
      "</ul>";
  } else {
    document.getElementById("data").innerHTML =
...
function initMap() {
  ...
  if (wms_services_emissions.length && wms_services_stations.length)
    showWmsLayer(0);
}
...
function showStreetView(coordinates) {
  streetViewMapDiv.hidden = false;
  document.getElementById("streetview_text").hidden = true;
  streetViewMap.setOptions({ position: coordinates, visible: true });
}

/* Función para ocultar el streetView */
function hideStreetView() {
  streetViewMapDiv.hidden = true;
  streetViewMap.setOptions({ visible: false });
  document.getElementById("streetview_text").hidden = false;
}
...
function distanceClick() {
  if (document.getElementById("distances").checked) {
    ...
    document.getElementById("distanceInfo").innerText =
      "Pulsa sobre dos estaciones para conocer la distancia que las separa";
  } else {
    if (poly) poly.setMap(null);
...
function getElevation(event) {
  ...
  elevationService.getElevationForLocations(positionalRequest, function (
    results,
    status
  ) {
    if (status == google.maps.ElevationStatus.OK) {
      ...
      if (results[0]) {
        ...
        showInfoWindow(
          event,
          elevation.toString().replace(".", ",") + " metros."
        );
      } else {
...
function showInfoWindow(event, elevation) {
  infoWindow.setContent(
    "<b>" + event.featureData.name + "</b><br/><b>Elevación:</b> " + elevation
  );
